import logging
import pickle
from .utils import parse_markdown_to_tree
from .cluster_tree_builder import ClusterTreeBuilder, ClusterTreeConfig
from .new_tree_builder import NewTreeBuilder,NewTreeConfig
...
supported_tree_builders = {"cluster": (ClusterTreeBuilder, ClusterTreeConfig),"markdown":(NewTreeBuilder,NewTreeConfig)}
...
class RetrievalAugmentation:
    ...
    def __init__(self, config=None, tree=None):
        ...
        self.tree_builder_type = config.tree_builder_type
    ...
    def add_documents(self, docs):
        ...
        if self.tree is not None:
            ...
            if user_input.lower() == "y":
                ...
                return
        if self.tree_builder_type == "markdown":
            tree_text = parse_markdown_to_tree(docs)
            self.tree_builder.add_text_tree(tree_text)
            self.tree = self.tree_builder.build_from_md_text()
        else:
    ...
    def answer_question(
        ...
        ## 将非叶子节点去除，进使用叶子节点作为上下文
        chunk_list = context.split("\n\n")
        leaves_text =""
        leaves=[]
        for i,node in enumerate(layer_information):
            if node["layer_number"] ==0:
                leaves_text+=chunk_list[i]
                leaves_text+="\n\n"
                leaves.append(node)


        # answer = self.qa_model.answer_question(leaves_text, question)
        #
        # if return_layer_information:
        #     return answer, leaves, leaves_text
