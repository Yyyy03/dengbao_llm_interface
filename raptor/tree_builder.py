...
class TreeBuilder:
    ...
    def build_from_md_text(self) -> Tree:

        all_nodes = dict()
        leaf_nodes=dict()
        layer_to_nodes = dict()
        root = self.construct_tree(all_nodes,all_nodes,layer_to_nodes)
        for index,node in all_nodes.items():
            if node.isleaf:
                leaf_nodes[index] = copy.deepcopy(node)
        tree = Tree(all_nodes,root,leaf_nodes,self.num_layers,layer_to_nodes)
